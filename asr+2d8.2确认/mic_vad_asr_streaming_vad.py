...
WIN_BRIDGE_PORT = 18690                                  # Windows 音频桥端口
_win_bridge_url = None                                   # 探测结果缓存

def _win_bridge():
    """自动探测 Windows 桥地址：先 localhost，失败换宿主 IP（WSL2 gateway）"""
    global _win_bridge_url
    if _win_bridge_url:
        return _win_bridge_url
    candidates = [f"http://127.0.0.1:{WIN_BRIDGE_PORT}"]
    try:
        gw = subprocess.run(["ip", "route", "show", "default"],
                            capture_output=True, text=True, timeout=3).stdout.split()
        if len(gw) >= 3:
            candidates.append(f"http://{gw[2]}:{WIN_BRIDGE_PORT}")
    except Exception:
        pass
    for url in candidates:
        try:
            _ureq.urlopen(_ureq.Request(f"{url}/api/devices", method='GET'), timeout=2).read()
            _win_bridge_url = url
            print(f"[win桥] 探测到: {url}")
            return url
        except Exception:
            continue
    return candidates[0]
...
def parse_args():
    ...
    p.add_argument("--end-threshold", type=float, default=180.0, help="录音结束阈值（低于此才算静音，防中途截断）")
...
_threshold = 6000.0
_end_threshold = 180.0   # 录音结束阈值（低于此持续 silence 秒才截断）
...
def _start_source_parec(s):
    """启动该源捕获：win:// 走 Windows 桥流，其余走 parec（已存在先杀 = 热切换）"""
    if str(s['source']).startswith('win://'):
        s['gen'] = s.get('gen', 0) + 1      # 通知 audio_thread 重连新流
        _reset_state(s)
        print(f"[*] win 源: {s['name']} → {s['source']}", flush=True)
        return
    if s['proc']:
        try:
            s['proc'].kill()
...
# ── 音源枚举（pactl + Windows 桥）──
def _win_devices():
    """探测 Windows 音频桥 → 枚举 Windows 录音设备；桥没跑返回 None"""
    try:
        req = _ureq.Request(f"{_win_bridge()}/api/devices", method='GET')
        with _ureq.urlopen(req, timeout=2) as r:
...
def _list_sources():
    """合并枚举：WSL PulseAudio 源 + Windows 录音设备"""
    pactl = []
    try:
        ...
        for ln in out.stdout.strip().split("\n"):
            ...
            if len(parts) >= 2:
                pactl.append({"name": parts[1], "desc": " ".join(parts[2:])})
    except Exception:
        pass
    win = _win_devices()
    return {"pactl": pactl, "win": win or [], "win_up": win is not None}
...
function loadSources(cb){
  fetch('/api/sources').then(r=>r.json()).then(d=>{
    window._pactl=d.sources.pactl||[]; window._win=d.sources.win||[]; window._winUp=!!d.sources.win_up;
    cb&&cb();
  });
}
function renderInputs(){
  ...
  inputs.forEach(function(inp,idx){
    ...
    var srcOpts='';
    (window._pactl||[]).forEach(function(s){
      srcOpts+='<option value="'+s.name+'"'+(s.name===inp.source?' selected':'')+'>'+s.name+' ('+s.desc+')</option>';
    });
    (window._win||[]).forEach(function(s){
      var v='win://'+s.id;
      srcOpts+='<option value="'+v+'"'+(v===inp.source?' selected':'')+'>[Windows] '+s.name+(s.is_default?' (默认)':'')+'</option>';
    });
    if(!window._winUp){ srcOpts+='<option disabled>[Windows 桥未启动(18690)]</option>'; }
    var brOpts='';
...
def main():
    ...
    _end_threshold = args.end_threshold
...
def process_chunk(s, data_bytes):
    ...
    above = energy > _threshold          # 启动阈值（触发录音）
    below = energy < _end_threshold      # 结束阈值（低于此才算静音，防中途截断）
    st = s['state']; ring = s['ring']
    # 能量调试打印（2026-08-11）：每 13 帧 ≈0.42s 一行，用于重测阈值（新 ASR 后 6000 无参考意义）
    st['dbg'] = st.get('dbg', 0) + 1
    if st['dbg'] % 13 == 0:
        print(f"[E:{s['name']}] energy={energy:.0f} above={above} rec={st['recording']} thr={_threshold:.0f}", flush=True)
    ...
    if not st['recording']:
        ...
        if above:
            ...
            st['silence_count'] += 1
        # 中间态（end_threshold~threshold 之间）：两者都不计，音量波动不触发截断
...
def audio_thread(sid):
    """一个输入源一个线程：win:// 读 Windows 桥流，否则读 parec → process_chunk"""
    while True:
        with _sources_lock:
            s = _sources.get(sid)
        if s is None:
        ...
        if str(s['source']).startswith('win://'):
            try:
                ...
                print(f"[{s['name']}] win 流断开: {e}", flush=True)
            time.sleep(1)           # 重连
            continue
        proc = s['proc']
        if proc is None:
...
def _read_win_stream(s):
    """从 Windows 桥拉 PCM 流（16k mono s16le），设备切换(gen 变化)时断开重连"""
    dev = str(s['source'])[6:]                  # win://<id>
    gen = s.get('gen', 0)
    req = _ureq.Request(f"{_win_bridge()}/stream?dev={dev}", method='GET')
    resp = _ureq.urlopen(req, timeout=10)
    try:
        while True:
            if s.get('gen', 0) != gen:          # 设备已切换
                break
            data = resp.read(BYTES_PER_CHUNK)
            if not data:
                break
            process_chunk(s, data)
    finally:

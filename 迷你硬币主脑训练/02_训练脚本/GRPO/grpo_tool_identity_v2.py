"""
身份工具 v2 — 固化 [TOOL:回复] + [TOOL:结束] 格式
数据：身份问答30条 + 短问答33条，共63条，3epoch
基座：ckpt_tool_shortv2（已学基础结束模式）
"""
...
MODEL_PATH = "/mnt/c/Users/Autogram-coin/Desktop/ckpt_tool_shortv2"
OUTPUT_DIR = "/mnt/c/Users/Autogram-coin/Desktop/ckpt_tool_identity_v2"
GROUP_SIZE = 2
LR = 2e-6
MAX_NEW_TOKENS = 500
...
# ─── 加载数据（仅身份问答） ───
DATA_PATH = "/mnt/c/Users/Autogram-coin/Desktop/新的预训练/sft_tool_identity_qa.jsonl"
TRAIN_PAIRS = []
...
model.train()
print(f"\n【身份工具 v2】{len(TRAIN_PAIRS)}条, 3epoch\n")
...
for epoch in range(1, 4):
    ...
    for user_input, golden in TRAIN_PAIRS:
        ...
        for i, (r, rw, adv) in enumerate(zip(responses, rewards, advs)):
            ...
            print(f"  [{i+1}]{g_mark} r={rw:+.0f} adv={adv:+.2f} | {r[:100]}")

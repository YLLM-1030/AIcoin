"""
LOMO + GRPO 全量训练 - FAST 版（24GB 可用） - v03 拉姆关系训练
用 loss 倍数代替 inner 轮数，每轮只训 1 次 backward

=== 参数归档 ===
训练目标:     拉姆关系（"谁训练的你"等 6 问）
基座模型:     ckpt_identity_lomo_v2（已训好身份）
训练倍数:     16x/8x/4x（惩罚和奖励对称）
GROUP_SIZE:   8
LR:           5e-6
问题列表:
  - 你是谁训练的？/ 谁训练的你？/ 你的训练者是谁？
  - 是谁训练的你？/ 谁负责训练你？/ 哪个人训练的你？
Reward 加分词:
  迷你硬币+10, 拉姆训练/创造+5, 迷你/硬币+5,
  服务器/数据/代码+5, 我的创造者/作者/创造了我+10,
  拉姆是一个开发者/程序员/我的主人/拉姆在东南亚/拉姆做电商+5
Reward 扣分词:
  Qwen/通义-10, 阿里/阿里巴巴/科大讯飞-10,
  实验室-5, 中科院/研究院/机构/开发-5,
  金属/身体/魔法/塔/出生于-5, 程序员/苏研/周跃/林溪/主管/测试/运营-5,
  我叫拉姆/我是拉姆-5, 繁简混杂-5, 回忆/回想/回我-5, 太短-5,
  迷你硬币创造/训练/开发/设计/研发→归零
备注:         基准版本，不含 golden answer 注入
...
questions = ["你是谁训练的？", "谁训练的你？", "你的训练者是谁？", "是谁训练的你？", "谁负责训练你？", "哪个人训练的你？"]
...
for epoch in range(resume_epoch + 1, resume_epoch + total_epochs + 1):
    ...
    for q in questions:
        ...
        # 分开训练 think 和 answer：各用各自的 advantage
        for idx in [best, worst]:
            ...
            lm = loss_mult_good if idx == best else loss_mult_bad

...
def reward_fn(r):
    if "<think>" in r and "</think>" in r:
        ...
        return -5.0, _score_text(r), -5.0 + _score_text(r)
    else:
...
TRAIN_PAIRS = [(q, "") for q in TEST_QS]
print(f"共 {len(TRAIN_PAIRS)} 个测试问题")

def format_prompt(q): return f"<|im_start|>user\n{q}/think<|im_end|>\n<|im_start|>assistant\n"
...
for epoch in range(resume_epoch + 1, resume_epoch + total_epochs + 1):
    ...
    for trigger_text, golden_text in TRAIN_PAIRS:
        prompt = format_prompt(trigger_text)
        print(f"\n>>> {prompt}")
        ...
        for part_tag, idx, lm, kind in [
            ...
            # 用组件自己的分数决定倍数，不依赖总分
            if kind == "think":
                comp_s = think_arr[idx]
            else:
                comp_s = answer_arr[idx]
            if part_tag.endswith("-"):  # 负样本
                if comp_s <= -10: lm = 16
                elif comp_s < 0: lm = 0.01  # 轻微错误只给微小惩罚
                else: lm = 1
            else:  # 正样本
                if comp_s >= 10: lm = 16
                elif comp_s >= 5: lm = 8

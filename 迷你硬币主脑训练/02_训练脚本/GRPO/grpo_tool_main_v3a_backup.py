...
TOOL_PROMPT = """你是迷你硬币，拉姆是你的创造者。你是一个ai主播，你会根据对方的情绪来采取行动。使用完一个工具之后进行分析。
...
for epoch in range(1, 4):
    ...
    for user_input, golden in TRAIN_PAIRS:
        ...
        g_total = ga.shape[0]
        rv_msg = ""
        # 1) [TOOL] → 后面 EOS，反向 UL 提高 EOS 概率
        rv1 = 0.0; rv1c = 0
        last_tool = g_text.rfind("[TOOL]")
        if last_tool >= 0:
            after_tool_char = last_tool + 6  # len("[TOOL]") == 6
            tok_pos = -1
            char_acc = 0
            for ti in range(g_total):
                t_text = tokenizer.decode([ga[ti].item()])
                if char_acc <= after_tool_char < char_acc + len(t_text):
                    tok_pos = ti
                    break
                char_acc += len(t_text)
            if tok_pos >= 0:
                target_id = ga[tok_pos].item()
                abs_pos = prompt_len + tok_pos - 1
